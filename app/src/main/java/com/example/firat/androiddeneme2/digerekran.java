...
/**
 * Created by Firat on 29.12.2014.
 */
public class digerekran extends Activity {
    ...
    public void btnYazdirOnClick(View v) {
        ...
        helper.yazLog(Helper.logTypes.debug, "Tag Count", "Tag Count: " + db.getAllTags().size());
        ...
        helper.yazLog(Helper.logTypes.error, "Todo Count", "Todo count: " + db.getToDoCount());
        ...
        helper.yazLog(Helper.logTypes.debug, "Get Tags", "Getting All Tags");

        List<Tag> allTags = db.getAllTags();
        for (Tag tag : allTags) {
            helper.yazLog(Helper.logTypes.debug, "Tag Name", tag.getTagName());
        }

        // Getting all Todos
        helper.yazLog(Helper.logTypes.debug, "Get Todos", "Getting All ToDos");

        List<Todo> allToDos = db.getAllToDos();
        for (Todo todo : allToDos) {
            helper.yazLog(Helper.logTypes.debug, "ToDo", todo.getNote());
        }

        // Getting todos under "Watchlist" tag name
        helper.yazLog(Helper.logTypes.debug, "ToDo", "Get todos under single Tag name");

        List<Todo> tagsWatchList = db.getAllToDosByTag(tag3.getTagName());
        for (Todo todo : tagsWatchList) {
            helper.yazLog(Helper.logTypes.debug, "ToDo Watchlist", todo.getNote());
        }

        // Deleting a ToDo
        helper.yazLog(Helper.logTypes.debug, "Delete ToDo", "Deleting a Todo");
        helper.yazLog(Helper.logTypes.debug, "Tag Count", "Tag Count Before Deleting: " + db.getToDoCount());

        db.deleteToDo(todo8_id);

        helper.yazLog(Helper.logTypes.debug, "Tag Count", "Tag Count After Deleting: " + db.getToDoCount());

        // Deleting all Todos under "Shopping" tag
        helper.yazLog(Helper.logTypes.debug, "Tag Count", "Tag Count Before Deleting 'Shopping' Todos: " + db.getToDoCount());

        db.deleteTag(tag1, true);

        helper.yazLog(Helper.logTypes.debug, "Tag Count", "Tag Count After Deleting 'Shopping' Todos: " + db.getToDoCount());
